...
function QuickOrder({ flavors, onCreated }: { flavors: Flavor[]; onCreated: (order: Order) => void }) {
  const available = flavors.filter((flavor) => flavor.active);
  const [form, setForm] = useState({ name: "Cliente balcão", phone: "", status: "pendente", deliveryFee: 0 });
  ...
  async function submit(event: FormEvent) {
    ...
    try { const result = await post<{ order: Order; checkoutUrl: string }>("/admin/orders/quick", { ...form, paymentStatus: "aguardando_pagamento", items }); setCheckoutUrl(result.checkoutUrl || ""); setCopied(false); onCreated(result.order); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível criar o pedido."); }
  }
  return <section className="manager glass-card"><div className="section-title"><div><span>Venda presencial</span><h2>Novo pedido rápido</h2></div></div><Notice kind="success">Este pedido rápido já sai como <strong>não pago</strong> e gera o link da InfinitePay para você enviar ao cliente.</Notice>{error && <Notice kind="error">{error}</Notice>}{checkoutUrl ? <Notice kind="success"><div className="quick-checkout-links"><span>Pedido criado com link de pagamento.</span><div className="button-row"><a className="ghost-button" href={checkoutUrl} target="_blank" rel="noreferrer">Abrir InfinitePay</a><button type="button" className="ghost-button" onClick={async () => { try { await navigator.clipboard.writeText(checkoutUrl); setCopied(true); } catch { setCopied(false); } }}>{copied ? "Link copiado" : "Copiar link"}</button><a className="success-button" href={`https://wa.me/${String(form.phone || "").replace(/\D/g, "")}?text=${encodeURIComponent(`Olá, ${form.name}. Segue seu link para pagamento: ${checkoutUrl}`)}`} target="_blank" rel="noreferrer">Enviar no WhatsApp</a></div></div></Notice> : <Notice kind="error">O pedido foi criado, mas o link da InfinitePay não apareceu. Confira se a integração da InfinitePay está ativa nas configurações e se a variável INFINITEPAY_HANDLE está preenchida.</Notice>}<form className="auth-form" onSubmit={submit}><div className="manager-create"><input placeholder="Nome do cliente" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /><input placeholder="Telefone (opcional)" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /><label className="number-selector"><span>Taxa de entrega</span><QuantityControl value={form.deliveryFee} min={0} max={999} step={0.5} formatValue={currency} label="Taxa de entrega" onChange={(deliveryFee) => setForm({ ...form, deliveryFee })} /></label><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value })}><option value="pendente">Recebido</option><option value="em_preparacao">Em preparação</option><option value="saiu_entrega">Saiu para entrega</option><option value="entregue">Entregue</option></select></div><div className="quick-items">{items.map((item, index) => <div className="quick-item" key={index}><select value={item.flavorId} onChange={(event) => setItems(items.map((current, currentIndex) => currentIndex === index ? { ...current, flavorId: Number(event.target.value) } : current))}>{available.map((flavor) => <option value={flavor.id} key={flavor.id}>{flavor.name} · {currency(flavor.price)} · estoque {flavor.stock}</option>)}</select><QuantityControl value={item.quantity} min={1} max={999} onChange={(quantity) => setItems(items.map((current, currentIndex) => currentIndex === index ? { ...current, quantity } : current))} /><button type="button" className="danger-button" onClick={() => setItems(items.filter((_, currentIndex) => currentIndex !== index))}>Remover</button></div>)}</div><div className="button-row"><button type="button" className="ghost-button" onClick={() => setItems([...items, { flavorId: available[0]?.id || 0, quantity: 1 }])}>+ Outra linha</button><button className="primary-button" disabled={!items.length}>Criar pedido</button></div></form></section>;
}
