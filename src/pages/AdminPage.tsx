  ...
  return (
      ...
      {tab === "orders" && <>
        ...
        <div className="admin-order-grid">
          {visibleOrders.map((order) => <button type="button" className={`admin-order-row glass-card ${order.paymentStatus === "pago" ? "paid" : order.paymentStatus === "aguardando_pagamento" ? "unpaid" : ""}`} key={order.id} onClick={() => { setOpenOrder(order); setEditingOrder(false); }}><i /><span><strong>{order.customer.name}</strong><small>#{order.id} • {dateTime(order.createdAt)}</small></span><b className={`admin-order-status-pill status-${order.status}`}>{statusLabel[order.status]}</b><strong className="admin-order-total">{currency(order.total)}</strong></button>)}
        </div>
