  ...
  return (
    <section className="admin-page page-stack">
      <header className="admin-header glass-card">
        ...
        <div className="admin-header-actions"><button className={notifications ? "success-button" : "danger-button"} onClick={enableNotifications}>{notifications ? "Notificações ativadas" : "Ativar notificações"}</button><button className="ghost-button" onClick={async () => { await post("/auth/logout"); navigate("/admin/login"); }}>Sair</button></div>
      </header>
      ...
      <nav className="admin-tabs" aria-label="Áreas do painel">
        {(["orders", "quick", "flavors", "config", "customers", "analytics"] as Tab[]).map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{({ orders: "Pedidos", quick: "Pedido rápido", flavors: "Sabores", config: "Configurações", customers: "Clientes", analytics: "Análise" } as Record<Tab,string>)[item]}</button>)}
      </nav>
      ...
      {tab === "orders" && <>
        <div className="metrics-bar glass-card">
          ...
          ].map(([key, label, value]) => <button key={String(key)} className={filter === key ? "active" : ""} onClick={() => setFilter(String(key))}><span>{label}</span><strong>{value}</strong></button>)}
        </div>
        <div className="admin-order-grid">
          {visibleOrders.map((order) => <button className={`admin-order-row glass-card ${order.paymentStatus === "pago" ? "paid" : ""}`} key={order.id} onClick={() => { setOpenOrder(order); setEditingOrder(false); }}><i /><span><strong>{order.customer.name}</strong><small>#{order.id} · {dateTime(order.createdAt)}</small></span><b>{currency(order.total)}</b></button>)}
        </div>
      ...
      {tab === "flavors" && <FlavorManager flavors={data.flavors} onChange={(flavors) => setData({ ...data, flavors })} />}
      {tab === "config" && <ConfigManager config={data.config} onChange={(config) => setData({ ...data, config })} />}
      ...
      {openOrder && <dialog className="admin-order-dialog" open onClick={(event) => { if (event.target === event.currentTarget) { setOpenOrder(null); setEditingOrder(false); } }}>
        <article className="admin-order-detail glass-card">
          <header><div><span>Pedido #{openOrder.id}</span><h2>{openOrder.customer.name}</h2><p>{dateTime(openOrder.createdAt)}</p></div><button className="dialog-close" onClick={() => setOpenOrder(null)} aria-label="Fechar">×</button></header>
          ...
          <div className="status-actions"><button className={openOrder.paymentStatus === "pago" ? "active" : ""} onClick={() => void updateOrder(openOrder, { paymentStatus: openOrder.paymentStatus === "pago" ? "aguardando_pagamento" : "pago" })}>{openOrder.paymentStatus === "pago" ? "Marcar não pago" : "Marcar pago"}</button>{["pendente","em_preparacao","saiu_entrega","entregue"].map((status) => <button key={status} className={openOrder.status === status ? "active" : ""} onClick={() => void updateOrder(openOrder, { status })}>{statusLabel[status]}</button>)}<button className="danger-button" onClick={() => void updateOrder(openOrder, { status: "cancelado" })}>Cancelar</button></div>
          <div className="button-row"><button className="ghost-button" onClick={() => setEditingOrder((value) => !value)}>{editingOrder ? "Fechar edição" : "Editar pedido"}</button><button className="danger-button" onClick={async () => { if (!confirm(`Excluir definitivamente o pedido #${openOrder.id}?`)) return; await remove(`/admin/orders/${openOrder.id}`); setOpenOrder(null); setEditingOrder(false); await load(); }}>Excluir pedido</button></div>
...
function OrderEditor({ order, flavors, onSave }: { order: Order; flavors: Flavor[]; onSave: (order: Order) => void }) {
  ...
  return <section className="inline-order-editor">
    ...
    {error && <Notice kind="error">{error}</Notice>}
    <div className="manager-create"><input aria-label="Nome do cliente" value={customerName} onChange={(event) => setCustomerName(event.target.value)} /><input aria-label="Telefone" value={customerPhone} onChange={(event) => setCustomerPhone(event.target.value)} /><input aria-label="Valor da entrega" type="number" min="0" step="0.01" value={deliveryFee} onChange={(event) => setDeliveryFee(Number(event.target.value))} /></div>
...
function LoyaltyProgressEditor({ item, onReload }: { item: LoyaltyRow; onReload: () => void }) {
  ...
  return <article><div><strong>{item.name}</strong><span>{item.phone}</span></div><label>R$ 5<input type="number" min="0" max="9" value={progress5} onChange={(event) => setProgress5(Number(event.target.value))} /></label><label>R$ 7<input type="number" min="0" max="9" value={progress7} onChange={(event) => setProgress7(Number(event.target.value))} /></label><button className="ghost-button" onClick={async () => { await patch(`/admin/loyalty/customer/${item.customer_id}`, { progress5, progress7 }); onReload(); }}>Salvar</button></article>;
}
...
function FlavorManager({ flavors, onChange }: { flavors: Flavor[]; onChange: (flavors: Flavor[]) => void }) {
  ...
  async function create(event: FormEvent) {
    event.preventDefault(); const result = await post<{ flavor: Flavor }>("/admin/flavors", draft); onChange([...flavors, result.flavor]); setDraft({ name: "", price: 5, stock: 0, imageUrl: "" });
  }
  return <section className="manager glass-card"><div className="section-title"><div><span>Cardápio e estoque</span><h2>Gerenciamento de sabores</h2></div></div><form className="manager-create" onSubmit={create}><input placeholder="Nome do sabor" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} required /><input type="number" min="0" step="0.01" value={draft.price} onChange={(e) => setDraft({ ...draft, price: Number(e.target.value) })} required /><input type="number" min="0" step="1" value={draft.stock} onChange={(e) => setDraft({ ...draft, stock: Number(e.target.value) })} required /><input placeholder="Caminho da imagem" value={draft.imageUrl} onChange={(e) => setDraft({ ...draft, imageUrl: e.target.value })} /><button className="primary-button">Adicionar sabor</button></form><div className="flavor-manager-list">{flavors.map((flavor) => <FlavorEditor key={flavor.id} flavor={flavor} onSave={(next) => onChange(flavors.map((item) => item.id === next.id ? next : item))} />)}</div></section>;
}

function FlavorEditor({ flavor, onSave }: { flavor: Flavor; onSave: (flavor: Flavor) => void }) {
  const [value, setValue] = useState(flavor);
  return <article className="flavor-editor"><img src={value.imageUrl} alt="" width="64" height="64" /><input value={value.name} onChange={(e) => setValue({ ...value, name: e.target.value })} /><input type="number" step="0.01" min="0" value={value.price} onChange={(e) => setValue({ ...value, price: Number(e.target.value) })} /><input type="number" min="0" value={value.stock} onChange={(e) => setValue({ ...value, stock: Number(e.target.value) })} /><label className="switch-line"><input type="checkbox" checked={value.active} onChange={(e) => setValue({ ...value, active: e.target.checked })} />Ativo</label><button className="primary-button" onClick={async () => { const result = await patch<{ flavor: Flavor }>(`/admin/flavors/${value.id}`, value); onSave(result.flavor); }}>Salvar</button></article>;
}

function ConfigManager({ config, onChange }: { config: StoreConfig; onChange: (config: StoreConfig) => void }) {
  const [value, setValue] = useState(config); const [saved, setSaved] = useState(false);
  ...
  return <section className="manager glass-card"><div className="section-title"><div><span>Operação única</span><h2>Configuração da loja</h2></div></div>{saved && <Notice kind="success">Configurações salvas.</Notice>}<div className="config-list"><label>Nome da loja<input value={value.storeName} onChange={(e) => setValue({ ...value, storeName: e.target.value })} /></label>{toggles.map(([key,label]) => <label className="config-toggle" key={key}><span>{label}</span><input type="checkbox" checked={Boolean(value[key])} onChange={(e) => setValue({ ...value, [key]: e.target.checked })} /></label>)}<label>Valor da entrega<input type="number" min="0" step="0.01" value={value.deliveryFee} onChange={(e) => setValue({ ...value, deliveryFee: Number(e.target.value) })} /></label><label>WhatsApp da loja<input value={value.whatsappNumber} onChange={(e) => setValue({ ...value, whatsappNumber: e.target.value })} /></label><label>Chave Pix<input value={value.pix?.key || ""} onChange={(e) => setValue({ ...value, pix: { key: e.target.value, name: value.pix?.name || "", bank: value.pix?.bank || "" } })} /></label><button className="primary-button" onClick={async () => { const result = await patch<{ config: StoreConfig }>("/admin/config", { ...value, pixKey: value.pix?.key || "", pixName: value.pix?.name || "", pixBank: value.pix?.bank || "" }); setValue(result.config); onChange(result.config); setSaved(true); }}>Salvar configurações</button></div><AdminPasswordForm /></section>;
}
